...
// Decompressed size declared in a zstd frame header (zstd_compress records
// it), or UINT64_MAX if unknown / on error / when zstd is not built in.
// O(1) — reads only the frame header — so a caller can validate a known
// size against the frame before committing to a large output allocation.
