...
//
// engine/ui/imm/Imm.cpp
//
// Lane 21 — immediate-mode UI implementation.
//
// GX Wave-B refactor: CPU framebuffer writes are gone.  All drawing now
// pushes batched GPU geometry via detail::GpuBatch.  The public API
// (begin_frame / end_frame / widget calls) is unchanged in intent; the
...
void begin_frame(psynder::gpu::CmdBuffer*        cmd,
                 ...
                 f32 viewport_height) {
    auto& ctx = d::context();
    ctx.vp_width   = viewport_width;
    ctx.vp_height  = viewport_height;
    ctx.hot_id     = 0;
    ctx.frame_open = true;

    d::batch().begin(cmd, pipeline, vb, ib, viewport_width, viewport_height);
}
...
void filled_rect(math::Vec2 origin, math::Vec2 size, u32 rgba_colour) {
    auto& ctx = d::context();
    if (!ctx.frame_open) return;
    d::filled_rect(origin, size, rgba_colour);
}

void rect_outline(math::Vec2 origin, math::Vec2 size, u32 rgba_colour) {
    auto& ctx = d::context();
    if (!ctx.frame_open) return;
    d::rect_outline(origin, size, rgba_colour);
}

void line(math::Vec2 a, math::Vec2 b, u32 rgba_colour) {
    auto& ctx = d::context();
    if (!ctx.frame_open) return;
    d::line(a, b, rgba_colour);
}

void label(math::Vec2 position, std::string_view text, u32 rgba_colour) {
    auto& ctx = d::context();
    if (!ctx.frame_open) return;
    d::draw_text(static_cast<i32>(std::lround(position.x)),
...
bool button(math::Vec2 position, math::Vec2 size, std::string_view text) {
    ...
    if (ctx.frame_open) {
        d::draw_button(position, size, text,
...
void render_graph_box(math::Vec2 origin, math::Vec2 size, std::string_view caption) {
    d::filled_rect(origin, size, d::theme().graph_bg);
    d::rect_outline(origin, size, d::theme().graph_axis);
    if (!caption.empty()) {
        d::draw_text(static_cast<i32>(origin.x) + 2,
...
void plot_polyline(math::Vec2 origin,
                   math::Vec2 size,
    ...
    for (u32 i = 1; i < n; ++i) {
        ...
        d::line(prev, cur, d::theme().graph_line);
        prev = cur;
    }
}
...
void graph(math::Vec2 origin,
    ...
    render_graph_box(origin, size, caption);
    plot_polyline(origin, size, std::span<const f32>{linear.data(), count}, max_ms);
}
...
void graph_series(math::Vec2 origin,
    ...
    if (!ctx.frame_open) return;
    render_graph_box(origin, size, caption);
    plot_polyline(origin, size, samples, max_value);
}
...
void selection_highlight(math::Vec2 origin, math::Vec2 size) {
    ...
    if (!ctx.frame_open) return;
    ++g_frame_tick;
    const u32 phase  = g_frame_tick / 4U;
    const u32 colour = d::theme().selection;

    // Emit dashed outline as individual pixel quads.
    auto plot_dashed_h = [&](i32 x0, i32 x1, i32 y) {
        for (i32 x = x0; x <= x1; ++x) {
            if (((static_cast<u32>(x) + phase) >> 2U) & 1U) {
                d::batch().push_line_pixel(x, y,     colour);
                d::batch().push_line_pixel(x, y + 1, colour);
            }
        }
    };
    auto plot_dashed_v = [&](i32 x, i32 y0, i32 y1) {
        for (i32 y = y0; y <= y1; ++y) {
            if (((static_cast<u32>(y) + phase) >> 2U) & 1U) {
                d::batch().push_line_pixel(x,     y, colour);
                d::batch().push_line_pixel(x + 1, y, colour);
            }
        }
    };
...
void brush_preview(math::Vec2 centre, f32 radius, f32 falloff_radius) {
    auto& ctx = d::context();
    if (!ctx.frame_open) return;
    d::blended_disk(centre, radius, d::theme().brush_fill);
    d::circle(centre, radius, d::theme().brush_outline, false);
    if (falloff_radius > radius) {
        d::circle(centre, falloff_radius, d::theme().brush_outline, false);
    }
}
...
f32 dist_point_to_segment_2d(math::Vec2 p, math::Vec2 a, math::Vec2 b) noexcept {
    ...
    if (ab_len2 <= 0.0f) return std::sqrt(apx * apx + apy * apy);
...
void draw_arrow(math::Vec2 base, math::Vec2 tip, u32 colour) {
    d::line(base, tip, colour);
    ...
    const f32 uy = dy * inv;
    const f32 head = 8.0f;
    constexpr f32 kCos = 0.906308f;
    constexpr f32 kSin = 0.422618f;
    ...
    d::line(tip, left,  colour);
    d::line(tip, right, colour);
}
...
GizmoAxis gizmo_translate(const GizmoProjection& proj) {
    auto& ctx = d::context();
    if (!ctx.frame_open) return GizmoAxis::None;
    ...
    auto try_pick = [&](GizmoAxis axis, math::Vec2 tip) {
        const f32 dd = dist_point_to_segment_2d(ctx.input.mouse, proj.origin, tip);
        if (dd < best) { best = dd; picked = axis; }
    };
    ...
    draw_arrow(proj.origin, ex, axis_colour(GizmoAxis::X, d::theme().gizmo_x));
    draw_arrow(proj.origin, ey, axis_colour(GizmoAxis::Y, d::theme().gizmo_y));
    draw_arrow(proj.origin, ez, axis_colour(GizmoAxis::Z, d::theme().gizmo_z));
    return picked;
}
...
GizmoAxis gizmo_rotate(const GizmoProjection& proj) {
    ...
    for (const Ring& r : rings) {
        ...
        for (u32 i = 0; i <= kSegments; ++i) {
            ...
            if (i > 0) {
                d::line(prev, p, r.colour);
                const f32 dd = dist_point_to_segment_2d(ctx.input.mouse, prev, p);
                if (dd < best) { best = dd; picked = r.ax; }
            }
            prev = p;
        }
    }
    ...
    if (picked != GizmoAxis::None) {
        ...
        for (u32 i = 0; i <= kSegments; ++i) {
            ...
            if (i > 0) d::line(prev, p, d::theme().button_active);
            prev = p;
        }
    }
    return picked;
