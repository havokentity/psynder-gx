...
struct State {
    ...
    std::vector<f32>            per_voice_mono{};                // kMaxVoices*frames (resampler + HRTF input)
...
void mixer_pull(f32* out_stereo, u32 frames, void* /*user*/) noexcept {
    ...
    // lock-free against an immutable view of the voice array. We also capture,
    // per snapshot slot:
    //   * snapshot_slot — the voice-pool index, so the bus accumulation pass
    //     can look up its BusId via slot_bus() without re-locking voices_mu.
    //   * clip_ptrs      — the resolved AudioClip* for the HRTF render path
    //     (filled after the lock to avoid holding two mutexes at once).
